...
rule musitedeep_predict:
    ...
    shell:
        ...
        cd /MusiteDeep_Keras2.0/MusiteDeep_Keras2.0/MusiteDeep
        conda run -n musitedeep_env python predict.py \
            -input "/workspace/{input}" \
            -output "/workspace/{ANALYSIS_DIR}/phosphorylation/{TARGETS_FILE_STR}/{wildcards.orthogroup}" \
...
rule compare_musitedeep_pairs:
    """
    Compare predicted phosphorylation sites between orthologous protein pairs.
    """
    input:
    ...
    run:
        ...
        for protein1, protein2 in pairs_list:
            ...
            shell(f"python -m compare.ps_sites '{input.sort}' '{protein1}' '{protein2}' -o '{outfile1}'")
            shell(f"python -m compare.ps_sites '{input.y}' '{protein1}' '{protein2}' -o '{outfile2}'")
